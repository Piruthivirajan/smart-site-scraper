...
async function getBrowserInstance() {
    ...
    browser = await puppeteer.launch({
        args: ['--no-sandbox', '--disable-setuid-sandbox',  '--proxy-server=socks5://38.127.172.186:174',],
    });
    return browser;
}
...
async function fetchSearchResults(query) {
    ...
    try {
        ...
        await page.goto(`https://duckduckgo.com/?q=${safeQuery}&t=h_&ia=web`, { waitUntil: 'networkidle2' });
        const rawHTML = await page.content();
        //const rawHTML = await axios.get(`https://duckduckgo.com/?q=${safeQuery}&t=h_&ia=web`, {  headers: {
       ...
       /* $('*').contents().filter(function() { return this.type === 'comment'; }).remove();
        ...
        const urls = await page.evaluate(() => {
         ...
         /* let urls = [];
          $('li[data-layout="organic"] a[data-testid="result-extras-url-link"]').each((index, element) => {
            // Extract the href attribute of each link
            const link = $(element).attr('href');
            
            // Check if the link does not include 'wikipedia.org' and limit to the first 3 links
            if (!link.includes('wikipedia.org') && urls.length < 3) {
                urls.push(link);
            }
        });
        */
